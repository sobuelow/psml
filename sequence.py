...
# def calc_lambda_patchiness(seq,residues,beta=1.):
#     """ Patchiness """
#     lam = residues.lambdas[list(seq)].to_numpy()
#     N = len(seq)
#     xs = np.arange(N)

#     lambdamap = np.add.outer(lam,lam) / 2.
#     r = (np.abs(np.subtract.outer(xs,xs))+1)**beta

#     U = get_U(N,lambdamap,r)
#     U /= (N*(N-1) / 2 + N)
#     return U
...
class SeqFeatures:
    ...
    nu_file=None,ah_intgrl_map=None):
        self.seq = seq
        self.N = len(seq)
        ...
        # self.q_patch = calc_qpatch(seq,charge_termini=charge_termini)
